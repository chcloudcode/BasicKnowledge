...
public class ImplementstrStr {
    ...
    public int strStr(String haystack, String needle) {
        ...
        int m = 0;
        int j=0;
        while(m<father.length-child.length+1){
            int i = m;
            while(i<father.length&&j<child.length&&father[i]==child[j]){
                i++;
                j++;
            }
            if(j==child.length){
        ...
        if(m>=father.length-child.length+1) return -1;
        return m;
    }

    public static void main(String[] args) {
        ImplementstrStr i = new ImplementstrStr();
        String haystack="aabbccdd";
        String needle = "ee";
